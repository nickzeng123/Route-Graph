...
def good(gst, d, v, w):
    return v not in list(gst.adj[w]) and \
           w not in list(gst.adj[v]) and \
           graph.edges[v, w, 0]['length'] > 0 \
           and d + graph.edges[v, w, 0]['length'] < (1.1 * goal_dist)
...
def total_elevation_gain(gr, rt):
    # your code here
    eg = gr.nodes[rt[0]]['elevation']
    for k in range(len(rt) - 1):
        if gr.nodes[rt[k+1]]['elevation'] > gr.nodes[rt[k]]['elevation']:
            eg += (gr.nodes[rt[k + 1]]['elevation'] - gr.nodes[rt[k]]['elevation'])
    return eg + 2
...
routevertices = [e[1] for e in sorted_lst]
...
def shade_given_time(k, n):
    ...
    tup = tuple((int(x * 255) for x in col)) # why doesn't this work???
...
# add red end circle
folium.CircleMarker((endlon,endlat),
